...
export default function BlogCard({
  post,
  fallbackImages = DEFAULT_FALLBACKS,
  index = 0,
}: BlogCardProps) {
  const imageSrc: string =
    post.featured_image || fallbackImages[index % fallbackImages.length] || DEFAULT_FALLBACKS[0]!
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden">
        ...
        <div
          className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/10 to-transparent opacity-80 transition-opacity duration-300 group-hover:opacity-60"
          aria-hidden="true"
        />
        {post.category?.name && (
      ...
      <div className="flex flex-1 flex-col gap-6 px-7 pb-8 pt-7 sm:px-8">
        <div className="space-y-4">
          <time
            ...
            className="inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-blue-700 dark:bg-blue-500/20 dark:text-blue-200"
          >
          ...
          <h3 className="text-2xl font-semibold leading-tight text-gray-900 transition-colors duration-200 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-300">
            <Link
              href={`/blog/${post.slug}`}
              className="focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 dark:focus-visible:ring-blue-300"
            >
              {post.title}
            </Link>
          </h3>
          {post.excerpt && (
            <p className="line-clamp-4 text-base leading-relaxed text-gray-600 dark:text-gray-300">
              {post.excerpt}
            </p>
          )}
        </div>
        ...
        <div className="mt-auto flex items-center justify-between text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 sm:text-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {post.author?.full_name && (
              <span aria-label={`Author ${post.author.full_name}`}>{post.author.full_name}</span>
            )}
          </div>
